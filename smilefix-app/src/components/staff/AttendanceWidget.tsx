  ...
  return (
    ...
    >
      ...
      <div className="space-y-2">
        {attendance.length === 0 ? (
          <p className="text-center text-sm text-[var(--color-on-surface-variant)] py-6">
            {t('staff.noAttendanceRecords', { defaultValue: 'No attendance records for today.' })}
          </p>
        ) : (
          attendance.map((rec, i) => {
            const member = staffById.get(rec.employeeId)
            const displayName = member
              ? `${member.firstName} ${member.lastName}`.trim()
              : rec.staffName?.trim() || t('staff.unknownMember', { defaultValue: 'Staff member' })
            const roleLabel = member?.role ?? rec.staffRole
            const cfg = statusStyles[rec.status] ?? statusStyles.present
            return (
              <motion.div
                key={rec.id}
