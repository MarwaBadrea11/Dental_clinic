...
export default function AppointmentsScreen({ navigation }: any) {
  ...
  const fetchAppointments = useCallback(async (silent = false) => {
    ...
    try {
      ...
      // result is { appointments: [...], stats: {...} }
      const list = Array.isArray(result)
        ? (result as BackendAppointment[])
        : (result as any).appointments ?? [];
      setAppointments(list.map(adaptAppointment));
    } catch (err) {
      if (err instanceof ApiRequestError) {
        if (err.status === 0) {
          setFetchError(t('networkError'));
        } else if (err.status === 401 || err.status === 403) {
          // Token expired or insufficient permissions — show retry so user can reload
  ...
  // Re-fetch if patient was null on first focus and hydrates later
  useEffect(() => {
    if (patient?.id) fetchAppointments();
  }, [patient?.id]);
    ...
    return (
      <AnimatedCard index={index}>
        <View style={[cardStyles.card, {
          ...
          {/* Neon left accent bar — mirrors to right edge in RTL */}
          ...
          <View style={[cardStyles.header, {
            flexDirection: rowDir,
            [isRTL ? 'paddingRight' : 'paddingLeft']: 10,
          }]}>
            {/* Avatar */}
      ...
      <SafeAreaView style={styles.flex}>
        ...
        <Animated.View style={{
          ...
          <View style={[styles.tabRow, { flexDirection: rowDir }]}>
            ...
            {/* Book now pill — pushed to the logical end of the row */}
            <TouchableOpacity
              style={[styles.newBtn, { marginStart: 'auto' as any }]}
              onPress={() => navigation.navigate('Booking')}
              activeOpacity={0.85}
            >
              <LinearGradient
...
const styles = StyleSheet.create({
  ...
  pageTitle: {
    ...
    // textAlign is set inline via `align` — do NOT set a static value here
    // so RTL/LTR flipping works without fighting the stylesheet.
  },
  // flexDirection set inline via rowDir; flexWrap so "Book Now" pill wraps
  // gracefully on narrow screens instead of pushing the title off-screen.
  tabRow:  { alignItems: 'center', gap: 8, flexWrap: 'wrap' },
  tabBtn: {
...
const cardStyles = StyleSheet.create({
  ...
  accentBar: {
    ...
    // `left` vs `right` is set inline per isRTL — do NOT set either here
    // or it will override the inline position and clip on one side.
    shadowOffset: { width: 0, height: 0 },
    shadowOpacity: 0.70, shadowRadius: 8,
  },
  // paddingLeft/Right set inline per isRTL (paddingStart 10)
  ...
  // paddingLeft/Right applied inline per isRTL
  divider: { height: StyleSheet.hairlineWidth, marginVertical: 13 },

  detailsRow: { justifyContent: 'space-between' },
  detailCol:  { flex: 1 },
  ...
  // gap + paddingStart applied inline per isRTL
